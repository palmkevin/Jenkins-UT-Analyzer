"""Helpers to construct synthetic builds/results directly in the DB for analysis tests.

The lifecycle/diff/classification logic is exercised against hand-built build sequences (no Jenkins
...
from uta.models import Build, TestIdentity, TestResult
...
def make_build(
    ...
    *,
    ...
    """Create a complete (by default) build where each ``name`` has ``status`` in every track.
    ...
    # Mirror the ingest pipeline's totals (per-result-row counts) so views built on build.total_*
    # (the build-page header / failures-only heading, issue #157) see the real invariant:
    # total_passed + total_failed + total_skipped == the build's result-row count.
    build.total_passed = sum(1 for r in build.results if r.status in ("PASSED", "FIXED"))
    build.total_failed = sum(1 for r in build.results if r.status in ("FAILED", "REGRESSION"))
    build.total_skipped = sum(1 for r in build.results if r.status == "SKIPPED")
    session.flush()
    return build
