...
def test_override_reflected_in_a_view_on_next_load(client, factory):
    ...
    build = FIRST_BUILD + 11  # a complete build with 36 result rows
    # The demo seeds ui_row_limit=20, so the build page arrives already paginated (36 rows → 2
    # pages).
    assert "Page 1 of 2" in client.get(f"/builds/{build}").text

    client.post("/control/settings", data={"key": "ui_row_limit", "value": "1"})
    assert "Page 1 of 36" in client.get(f"/builds/{build}").text  # cap now bites — view reflects it
