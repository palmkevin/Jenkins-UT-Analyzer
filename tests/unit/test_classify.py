...
def _add_data(run, author=None, component=None):
...
def test_strong_code_match_outscores_weak_data_match(session_factory):
    ...
    with session_scope(session_factory) as s:
        ...
        _add_code(run, paths_json='[{"editType": "edit", "file": "/trunk/lx/ut_pkg/mod.py"}]')
        _add_data(run, author="THA", component="LXFOO_CSVC")
        s.flush()
        [c] = _classify(s, run)
        assert c.predicted_cause == PredictedCause.CODE_CHANGE
...
def test_strong_data_match_outscores_weak_code_match(session_factory):
    ...
    with session_scope(session_factory) as s:
        ...
        _add_code(run, paths_json='[{"editType": "edit", "file": "/trunk/lx/ut_pkg/other.py"}]')
        _add_data(run, author="THA")
        s.flush()
        [c] = _classify(s, run)
        assert c.predicted_cause == PredictedCause.DATA_CHANGE
...
def test_kb_provenance_boosts_confidence(session_factory):
    ...
    with session_scope(session_factory) as s:
        ...
        run1 = make_run(s, 1, {"t": "FAILED"}, errors={"t": ("boom", stack)})
        record_signatures_for_run(s, run1)
        analysis = apply_run(s, run1, baseline=None)
        s.flush()
        classify_run(s, run1, analysis.opened_episodes)
        ...
        run2 = make_run(s, 2, {"t": "PASSED"})
        apply_run(s, run2, baseline=run1)
        s.flush()
        run3 = make_run(s, 3, {"t": "FAILED"}, errors={"t": ("boom", stack)})
        _add_code(run3, paths_json='[{"editType": "edit", "file": "/trunk/lx/ut_pkg/mod.py"}]')
        record_signatures_for_run(s, run3)
        analysis3 = apply_run(s, run3, baseline=run2)
        s.flush()
        classify_run(s, run3, analysis3.opened_episodes)
        s.flush()
