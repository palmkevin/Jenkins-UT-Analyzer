...
def test_new_failure_opens_episode_and_sets_failing(session_factory):
    with session_scope(session_factory) as s:
        ...
        assert episodes[0].first_failure_build_id == build.id
        assert lc.all_time_first_failure_build_id == build.id
...
def test_fix_closes_episode(session_factory):
    with session_scope(session_factory) as s:
        ...
        assert ep.fixed_in_build_id == r2.id
...
def test_removed_then_reappearing_pass_closes_episode(session_factory):
    with session_scope(session_factory) as s:
        r1 = make_build(s, 1, {"t": "FAILED"})
        apply_build(s, r1, baseline=None)
        r2 = make_build(s, 2, {"other": "PASSED"})  # "t" absent -> REMOVED, episode open
        apply_build(s, r2, baseline=r1)
        r3 = make_build(s, 3, {"t": "PASSED", "other": "PASSED"})  # reappears passing
        analysis = apply_build(s, r3, baseline=r2)
        lc = _lc(s, "t")
        assert lc.state == LifecycleState.FIXED
        ep = _episodes(s, "t")[0]
        assert ep.is_open is False
        assert ep.fixed_in_build_id == r3.id
        assert get_identity(s, "t").id in analysis.diff.newly_fixed
        # Re-applying the same build stays idempotent: nothing open to reconcile.
        apply_build(s, r3, baseline=r2)
        eps = _episodes(s, "t")
...
def test_removed_then_reappearing_failure_continues_same_episode(session_factory):
    with session_scope(session_factory) as s:
        ...
        r2 = make_build(s, 2, {"other": "PASSED"})  # "t" absent -> REMOVED, episode open
        apply_build(s, r2, baseline=r1)
        r3 = make_build(s, 3, {"t": "FAILED", "other": "PASSED"})  # reappears still failing
        apply_build(s, r3, baseline=r2)
        lc = _lc(s, "t")
        assert lc.state == LifecycleState.FAILING
        ...
        assert eps[0].last_failing_build_id == r3.id
        assert eps[0].fixed_in_build_id is None
...
def test_still_failing_extends_episode_and_age(session_factory):
    with session_scope(session_factory) as s:
        r1 = make_build(s, 1, {"t": "FAILED"})
        apply_build(s, r1, baseline=None)
        r2 = make_build(s, 2, {"t": "FAILED"})
        apply_build(s, r2, baseline=r1)
        ep = _episodes(s, "t")[0]
        assert ep.last_failing_build_id == r2.id
        assert ep.age_builds == 2  # failed in two complete builds
...
def test_reapply_same_run_is_idempotent(session_factory):
    with session_scope(session_factory) as s:
        r1 = make_build(s, 1, {"t": "FAILED"})
        apply_build(s, r1, baseline=None)
        apply_build(s, r1, baseline=None)  # re-apply
