...
TUNABLES: tuple[Tunable, ...] = (
    Tunable(
        "flaky_transition_threshold", "Flaky threshold", "Flakiness", "float", 0.0, 1.0,
        "Oscillation score above which a test is flagged flaky (§3).",
    ),
    Tunable(
        "flaky_window_days", "Flaky window (days)", "Flakiness", "int", 1, 365,
        "How far back the flaky oscillation score looks (§3).",
    ),
    Tunable(
        "pgtrgm_similarity_cutoff", "Similarity cutoff", "Knowledge base", "float", 0.0, 1.0,
        "Minimum trigram similarity for a KB match to surface (§4).",
    ),
    Tunable(
        "kb_top_k", "KB top-k", "Knowledge base", "int", 1, 50,
        "How many similar past cases to surface per failure (§4).",
    ),
    Tunable(
        "recently_fixed_days", "Recently-fixed window (days)", "Triage", "int", 1, 90,
        "How long a fix stays in the §0 recently-fixed bucket.",
    ),
    Tunable(
        "ui_row_limit", "Dashboard row cap", "Triage", "int", 0, 5000,
        "Max rows a section renders before a 'Load all' link (0 disables the cap).",
    ),
    Tunable(
        "expected_shards", "Expected shards", "Ingest", "int", 1, 10,
        "Shards a run must report to be considered complete.",
    ),
    Tunable(
        "data_change_lookback_hours", "Data-change lookback (hours)", "Ingest", "int", 0, 168,
        "How far before a run's start to look for correlated data changes.",
    ),
    Tunable(
        "data_change_tolerance_minutes", "Data-change tolerance (min)", "Ingest", "int", 0, 120,
        "Clock-skew margin widening the data-change correlation window.",
    ),
    Tunable(
        "backfill_depth", "Back-fill depth", "Ingest", "int", 1, 500,
