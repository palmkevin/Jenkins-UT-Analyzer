...
Deterministic classification (``analyze/classify.py``) stays pure and offline; this step builds
**after** it, only for builds being analysed, and only when a real provider is supplied. It fills
...
so the offline gate and existing builds are untouched. Back-fill passes Noop; the poller passes the
...
from uta.models import Build, Classification, FailureSignature, TestIdentity, TestResult


def _failing_result_with_signature(
    session: Session, build: Build, identity_id: int
) -> TestResult | None:
    """The build's failing result for this test that carries a signature (the recurrence key)."""
...
def hypothesize_episode(
    ...
    *,
    ...
    result = _failing_result_with_signature(session, build, identity_id)
    if result is None:
        return False
    ...
    # Rank the build's change candidates against *this* failure so the prompt can lead with the
...
def hypothesize_build(
    ...
    *,
    ...
    """Hypothesise every episode this build newly opened. No-op under Noop. Returns the count
    written."""
    ...
    for identity_id, episode_id in opened_episodes:
        if hypothesize_episode(
            session, build, identity_id, episode_id, provider, top_k=top_k, cutoff=cutoff
        ):
            written += 1
    return written
