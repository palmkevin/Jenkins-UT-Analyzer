...
Because **every** build is triggered by a trunk commit, "fails then passes" is never "no change" —
...
- Build the test's pass/fail sequence from the builds in which it **actually produced a result**,
...
class FlakinessStats:
    failed_total: int  # all-time builds in which the test failed
    failed_in_window: int
    last_failed_at: datetime | None
    builds_in_window: int  # builds that produced a result in the window
    transitions: int
...
def _sequence(session: Session, identity_id: int) -> list[_BuildPoint]:
    """The test's per-build pass/fail points, oldest-first, only complete builds that produced a
    result.

    Tracks are collapsed: a build is a *fail* if the test FAILED in any track; a data point exists
    only if the test reported (passed or failed) in at least one track — absent/SKIPPED-only builds
    are holes, not points.
    """
    rows = session.execute(
        select(Build.id, Build.build_number, Build.started_at, TestResult.track, TestResult.status)
        .join(Build, Build.id == TestResult.build_id)
        .where(TestResult.test_identity_id == identity_id, Build.complete.is_(True))
        .order_by(Build.started_at, Build.id)
    ).all()

    by_build: dict[int, _BuildPoint] = {}
    order: list[int] = []
    for build_id, build_number, started_at, track, status in rows:
        if status not in _REPORTED:
            continue
        point = by_build.get(build_id)
        if point is None:
            point = _BuildPoint(_aware(started_at), build_number, False, frozenset(), frozenset())
            by_build[build_id] = point
    ...
    return [by_build[r] for r in order]
...
def compute_stats(
    ...
    *,
    ...
    builds_in_window = len(window)
    fails_in_window = sum(1 for p in window if p.failed)

    states = [p.failed for p in window]
    transitions = sum(1 for i in range(1, len(states)) if states[i] != states[i - 1])
    score = transitions / builds_in_window if builds_in_window else 0.0
    fail_rate = fails_in_window / builds_in_window if builds_in_window else 0.0
...
def history(
    ...
    *,
    ...
    and its test's flakiness card always agree on which builds are "in window".
    """
    now = now or _now()
    cutoff = now - timedelta(days=window_days)
    seq = _sequence(session, identity_id)
    return [
        {"number": p.number, "started_at": p.started_at, "failed": p.failed}
        for p in seq
...
def recompute_flaky_flags(
    ...
    *,
    ...
    Driven after each complete build is analysed. Only ever-failing tests have a lifecycle row, so
        ...
        rows.append(
            {
                ...
                "builds_in_window": stats.builds_in_window,
