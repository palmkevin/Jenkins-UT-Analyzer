...
def test_search(
    ...
    open — the test is failing now), ``removed`` (that open episode belongs to a test that has since
    disappeared from the suite — open, but not "failing now"), and ``last_closed_at`` (when the most
    recent episode closed, i.e. the test passed again — only when none is open).
    ``latest_failure_at`` is the most recent failure across all episodes and drives the default
    ``recent`` sort (newest-first). ``sort`` reorders the rows — ``recent`` (default), ``name`` or
    ``owner``.
    ...
    stmt = (
        ...
        .outerjoin(TestLifecycle, TestLifecycle.test_identity_id == TestIdentity.id)
    ...
    rows: list[dict] = []
    for ident, latest_failure_at, last_closed_at, open_count, lc_state in session.execute(stmt):
        has_open = bool(open_count or 0)
        # A removed test keeps an *open* episode (disappeared ≠ fixed) — flag it distinctly so it
        # isn't shown as "failing now" (mirrors the triage queue's Removed treatment).
        removed = has_open and lc_state == LifecycleState.REMOVED
        rows.append(
            {
                "identity_id": ident.id,
                "test_id": ident.canonical_name,
                # Displayed only — no pivot, the queue has no suite filter (#191).
                "suite": ident.suite,
                "owner": ident.main_developer,
                "owner_url": pivot_url("owner", ident.main_developer),
                ...
                # Only meaningful when the episode is closed — otherwise it's still open/removed.
